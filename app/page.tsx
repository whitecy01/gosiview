...
const ASSIGNEES: Assignee[] = ['사장님', '총무님', '원장님'];

const ASSIGNEE_STYLE: Record<Assignee, { bg: string; text: string }> = {
  '사장님': { bg: 'bg-rose-500/20',   text: 'text-rose-300' },
  '총무님': { bg: 'bg-amber-500/20',  text: 'text-amber-300' },
  '원장님': { bg: 'bg-teal-500/20',   text: 'text-teal-300' },
};
...
function AssigneeSelector({ value, onChange }: { value: Assignee; onChange: (a: Assignee) => void }) {
  return (
          ...
          <button
            ...
            className={`rounded-lg px-2.5 py-1 text-xs font-semibold transition-colors border ${
              active
                ...
                : 'bg-transparent text-gray-500 border-[#2A2A2A] hover:border-gray-500 hover:text-gray-300'
            }`}
          >
            {a}
          </button>
...
function TodoModal({
  ...
  onEdit: (id: string, text: string, assignee: Assignee) => void;
  onDelete: (id: string) => void;
  onToggle: (id: string) => void;
  onClose: () => void;
}) {
  const [input, setInput] = useState('');
  const [assignee, setAssignee] = useState<Assignee>('사장님');
  const [editId, setEditId] = useState<string | null>(null);
  const [editText, setEditText] = useState('');
  const [editAssignee, setEditAssignee] = useState<Assignee>('사장님');
  ...
  function submitAdd() {
    ...
    onAdd(input.trim(), assignee);
    setInput('');
  }

  function submitEdit() {
    if (!editText.trim() || !editId) return;
    onEdit(editId, editText.trim(), editAssignee);
    setEditId(null);
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-2 max-h-72">
          ...
          {todos.map((todo) => {
            const s = ASSIGNEE_STYLE[todo.assignee];
            return (
              <div key={todo.id} className="flex flex-col gap-1.5 rounded-xl border border-[#2A2A2A] bg-[#161616] px-3 py-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    {editId === todo.id ? (
                      ...
                      <button onClick={() => startEdit(todo)} className="rounded p-1 text-gray-500 hover:text-gray-300 hover:bg-[#222] transition-colors">
                        <Pencil size={13} />
                      </button>
                    )}
                ...
                {editId === todo.id ? (
                  <div className="pl-7">
                    <AssigneeSelector value={editAssignee} onChange={setEditAssignee} />
                  </div>
                ) : (
                  <div className="pl-7">
                    <span className={`rounded px-2 py-0.5 text-[11px] font-semibold ${s.bg} ${s.text}`}>
                      {todo.assignee}
                    </span>
                  </div>
                )}
        ...
        {/* Add input */}
        <div className="flex flex-col gap-2 border-t border-[#2A2A2A] px-6 py-4">
...
export default function TodoListPage() {
  ...
  const [todos, setTodos] = useState<Todo[]>([
    { id: uid(), date: '2026-04-01', text: '101호 에어컨 필터 청소',     done: true,  assignee: '총무님' },
    { id: uid(), date: '2026-04-01', text: '월세 미납자 연락 (205호)',    done: false, assignee: '원장님' },
    { id: uid(), date: '2026-04-01', text: '공과금 정산 자료 정리',       done: false, assignee: '사장님' },
    { id: uid(), date: '2026-04-03', text: '302호 도배 업체 견적 요청',   done: false, assignee: '총무님' },
    { id: uid(), date: '2026-04-03', text: '신규 입실자 열쇠 준비 (401호)', done: false, assignee: '원장님' },
    { id: uid(), date: '2026-04-05', text: '관리비 납부 마감 확인',       done: false, assignee: '사장님' },
    { id: uid(), date: '2026-04-07', text: '507호 배수구 수리 업체 예약', done: false, assignee: '총무님' },
    { id: uid(), date: '2026-04-07', text: '4월 도시가스 검침',           done: false, assignee: '원장님' },
    { id: uid(), date: '2026-04-10', text: '월세 납부 마감일',            done: false, assignee: '사장님' },
    { id: uid(), date: '2026-04-10', text: '203호 계약 갱신 상담',        done: false, assignee: '원장님' },
    { id: uid(), date: '2026-04-14', text: '소화기 점검',                 done: false, assignee: '총무님' },
    { id: uid(), date: '2026-04-15', text: '한전 검침 및 승계 처리',      done: false, assignee: '사장님' },
    { id: uid(), date: '2026-04-15', text: '106호 퇴실 청소 업체 예약',   done: false, assignee: '총무님' },
    { id: uid(), date: '2026-04-18', text: '공용 세탁기 필터 청소',       done: false, assignee: '원장님' },
    { id: uid(), date: '2026-04-21', text: '건물 외벽 점검',              done: false, assignee: '사장님' },
    { id: uid(), date: '2026-04-25', text: '다음 달 입실 예정자 확인',    done: false, assignee: '원장님' },
    { id: uid(), date: '2026-04-28', text: '302호 도배 완료 확인',        done: false, assignee: '총무님' },
    { id: uid(), date: '2026-04-30', text: '4월 수입/지출 정산',          done: false, assignee: '사장님' },
  ]);
  ...
  function addTodo(text: string, assignee: Assignee) {
    if (!selectedDate) return;
    setTodos(prev => [...prev, { id: uid(), date: selectedDate, text, done: false, assignee }]);
  }
  function editTodo(id: string, text: string, assignee: Assignee) {
    setTodos(prev => prev.map(t => t.id === id ? { ...t, text, assignee } : t));
  }
              ...
              >
                ...
                <div className="flex flex-col gap-1 overflow-hidden">
                  {dayTodos.slice(0, 3).map((todo) => {
                    const s = ASSIGNEE_STYLE[todo.assignee];
                    return (
                      <div key={todo.id} className={`flex items-center gap-1 rounded px-1.5 py-1 ${todo.done ? 'bg-gray-700/50' : 'bg-indigo-500/20'}`}>
                        <span className={`shrink-0 rounded px-1 text-[10px] font-bold ${s.bg} ${s.text}`}>
                          {todo.assignee.replace('님', '')}
                        </span>
                        <span className={`truncate text-xs leading-tight ${todo.done ? 'line-through text-gray-500' : 'text-indigo-300'}`}>
                          {todo.text}
                        </span>
                      </div>
                    );
